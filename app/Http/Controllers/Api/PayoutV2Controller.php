...
class PayoutV2Controller extends Controller
{
    ...
    {
        $apiId = $request->header('X-API-ID');

        if(!$apiId) {
            return $this->encryptedError($this->securityHelper($request), 'X-API-ID header is required', [], 400);
        }

        // P7X4K2 - Get Balance
        // M9Q8R5 - Initiate Payout (Single Payout)
        // B8Y2L7 - Bulk Payout
        // H5C9V4 - Get Payout status by merchant reference id
        // Z1D7P8 - Get Payout status by transaction id


        return match ($apiId) {
            'P7X4K2' => $this->getBalance($request),
            'M9Q8R5' => $this->initiate($request),
            'B8Y2L7' => $this->bulk($request),
            'H5C9V4' => $this->getStatus($request, 'merchant_reference_id'),
            'Z1D7P8' => $this->getStatus($request),
            default => $this->encryptedError($this->securityHelper($request), 'Invalid API endpoint', [], 404),
        };
    }
    ...
    public function getBalance(Request $request)
    {
        ...
        $response = $this->successResponse([
            'account_holder' => $virtualAccount->account_holder,
            'bank_name' => 'HDFC Bank',
            'ifsc_code' => $virtualAccount->ifsc,
            'van' => $virtualAccount->van,
            'balance' => $balance
    ...
    * }
     */
    public function getStatus(Request $request, string $type = 'transaction_id')
    {
        $securityHelper = $this->securityHelper($request);
        $user = $this->merchant($request);
        ...
        if($type === 'transaction_id') {
            ...
            $rules = ['merchant_reference_id' => 'required|string|max:255'];
        }

        $validator = Validator::make($body_data, $rules);
        ...
        if($type === 'transaction_id') {
            ...
            $payout = $user->payouts()->where('merchant_reference_id', $body_data['merchant_reference_id'])->first();
        }
    ...
    * }
    ...
    public function bulk(Request $request)
    {
        ...
        $validator = Validator::make($body_data, [
            ...
            'payouts.*.merchant_reference_id' => 'required|string|max:255|unique:payouts,merchant_reference_id',
            'payouts.*.account_holder'       => 'required|string|max:255',
            'payouts.*.account_number'       => 'required|string',
            'payouts.*.ifsc_code'            => 'required|string',
            'payouts.*.bank_name'            => ['required', 'string', 'regex:/^[A-Za-z\s&.-]+$/'],
        ...
        $dtos = array_map(fn($p) => new SinglePayoutDTO(
            merchantReferenceId: $p['merchant_reference_id'],
        ...
        $response = app(PayoutService::class)->createBulkPayout($dtos, $user);
        if($response['status'] !== 'success')
        {
            return $this->encryptedError($securityHelper, $response['message'] ?? 'Failed to initiate payout. Please try again.', [], 500);
        }
